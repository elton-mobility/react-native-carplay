 ...
 */
const withCarPlayAppDelegate = (config, options) => {
  return withAppDelegate(config, props => {
    ...
    return {
      ...props,
      modResults: { ...props.modResults, contents },
    };
  });
};
 ...
 */
function updateClassDefinition(contents) {
 ...
 */
function updateReactNativeDelegateType(contents) {
 ...
 */
function addObjcToReactNativeFactory(contents) {
 ...
 */
function restructureApplicationMethod(contents) {
 ...
 */
function removeExistingCarPlayMethods(contents) {
  const carPlayMethodsPattern = /\/\/\s*@generated begin carplay-applicationConfigForConnecting[\s\S]*?\/\/\s*@generated end carplay-applicationConfigForConnecting/;
 ...
 */
function addCarPlaySceneMethods(contents) {
  ...
  const universalLinksPattern = /\/\/\s*Universal Links[\s\S]*?restorationHandler:[^}]*\}\s*\)\s*(?:\|\|\s*result)?\s*\n  \}/;
...
module.exports = withCarPlayAppDelegate;
